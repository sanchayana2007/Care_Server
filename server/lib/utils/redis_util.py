...
class BaseRedisProtocol(LineReceiver, policies.TimeoutMixin):
    ...
    def publish(self, channel, message):
        ...
        """
        # Adding Project Code filter
        channel = '{0}_{1}'.format(
                    REDIS_SERVER_KEY,
                    channel
                )
...
class RedisSubscriberProtocol(RedisProtocol):
    ...
    def subscribe(self, channels):
        if isinstance(channels, (str, unicode)):
            # Adding Project Code filter
            channels = '{0}_{1}'.format(
                        REDIS_SERVER_KEY,
                        channels
                    )
            channels = [channels]
        try:
            self.messageReceived('SUBSCRIBE', str(channels[0]), None)
        except:
            Log.e('REDIS', 'CODE: 1668')
    ...
    def unsubscribe(self, channels):
        if isinstance(channels, (str, unicode)):
            ...
            channels = [channels]
        try:
            self.messageReceived('UNSUBSCRIBE', str(channels[0]), None)
        except:
            Log.e('REDIS', 'CODE: 1677')
        return self.execute_command("UNSUBSCRIBE", *channels)

    def psubscribe(self, patterns):
        if isinstance(patterns, (str, unicode)):
            # Adding Project Code filter
            patterns = '{0}_{1}'.format(
                        REDIS_SERVER_KEY,
                        patterns
                    )
            patterns = [patterns]
...
class RedisFactory(protocol.ReconnectingClientFactory):
    ...
    def delConnection(self, conn):
        try:
            ...
            Log.i('REDIS', "Could not remove connection from pool: %s" % str(e))
    ...
    def getConnection(self, put_back=False):
        ...
        while True:
            ...
            if conn.connected == 0:
                Log.i('REDIS', 'Discarding dead connection.')
            else:
                if put_back:
                    self.connectionQueue.put(conn)
...
class RedisSubscriberFactory(RedisFactory):
    protocol = RedisSubscriberProtocol
...
def RedisConnectionPool(host="localhost", port=6379, dbid=None,
...
__all__ = [
    ...
    RedisConnectionPool, lazyConnectionPool,
