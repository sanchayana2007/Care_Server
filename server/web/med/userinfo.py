...
@xenSecureV1
class MedServiceInfoHandler(cyclone.web.RequestHandler,
            ...
            if len(profile):
                ...
                if len(app):
                    ...
                    if app[0]['apiId'] in [ 502020, 502022]: # TODO: till here
                        if self.apiId == 502022:
                            appFind = yield self.applications.find(
                                        {
                                            "apiId" : 502020
                                        }
                                    )
                            ...
                            for pro in proFind:
                                ...
                                serviceBookCount = yield self.serviceBook.count(
                                            {
                                                'profileId':pro['_id']
                                            }
                                        )
                                if len(accFind):
                                    for acc in accFind:
                                        v = {
                                                ...
                                                'phoneNumber':acc['contact'][0]['value'],
                                                'bookingCount':serviceBookCount
                                            }
                                        result.append(v)
                            code = 2000
                            status = True
