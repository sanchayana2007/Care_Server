...
@xenSecureV1
class MedServiceProviderServiceHandler(cyclone.web.RequestHandler,
                ...
                if len(app):
                    ...
                    if app[0]['apiId'] in [ 502021, 502022]: # TODO: till here
                        ...
                        if self.apiId == 502021:
                            ...
                            try:
                                serId = ObjectId(self.request.arguments['serId'][0])
                            except:
                                code = 7810
                                status = False
                                message = "Invalid Service Provider Id"
                                raise Exception
                            providerServiceList = yield self.serviceProviderServices.find({'serviceProviderId':serId})
                            if len(providerServiceList):
                                for res in providerServiceList:
                                    v = {
                                            'status':res['status'],
                                            'providerService':str(res['_id'])
                                        }
                                    ...
                                    if len(serInfo):
                                        v['serviceName'] = serInfo[0]['serNameEnglish']
                                        if len(serInfo[0]['media']):
                                            for docx in serInfo[0]['media']:
                                                docx['link'] = self.fu.serverUrl + '/uploads/' \
                                                + str(self.entityId)\
                                                + '/service_media/' + str(serInfo[0]['_id'])\
                                                + '/' + str(docx['time']) + docx['mimeType']
                                        else:
                                            img = {
                                                    'link':"https://medix.xlayer.in/uploads/default/serviceList.png"
                                                    }
                                            serInfo[0]['media'].append(img)
                                        v['media'] = serInfo[0]['media']
                                        result.append(v)
                                code = 2000
                                status = True
