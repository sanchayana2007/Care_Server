...
@xenSecureV1
class MedServiceDistrictInfoHandler(cyclone.web.RequestHandler,
                ...
                if len(app):
                    ...
                    if app[0]['apiId'] in [ 502020, 502021,502022]: # TODO: till here
                        if self.apiId == 502021:
                            try:
                                state = self.request.arguments['state'][0]
                            except:
                                code = 4658
                                status = False
                            ...
                            if state != "UTTAR PRADESH":
                                code = 3000
                                status = False
                                message = "Service is currently not available for " + state
                                raise Exception
                            districtList = yield self.pincode.distinct("district")
                            for res in districtList:
                                v = {
                                        'districtName':res
                                    }
                                result.append(v)
