...
@xenSecureV1
class ServiceAreaHandler(cyclone.web.RequestHandler, MongoMixin):
            ...
            if len(profile):
                ...
                if len(app):
                    if app[0]['apiId'] == 20216: # TODO: till here
                        ...
                        if vTypeId != None:
                            vTypes = yield self.serviceArea.find(
                                        {
                                            '_id': vTypeId,
                                            'entityId': self.entityId,
                                            'disabled': tDisabled
                            ...
                            if len(vTypes):
                                for v in vTypes:
                                    ...
                                    del v['entityId']
                                    area = []
                                    for idx, val in enumerate(v['area']['coordinates'][0]):
                                        area.append(
                                                    {
                                                        'lng': val[0],
                                                        'lat': val[1]
            ...
            if len(profile):
                ...
                if len(app):
                    if app[0]['apiId'] == 20216: # TODO: till here
                        ...
                        sArea = self.request.arguments.get('area')
                        code, message = Validate.i(
                                    sArea,
                                    'Area ',
                                    dataType=list,
                        ...
                        for i, val in enumerate(sArea):
                            try:
                                ...
                                Log.d('SERVICE-AREA', e)
                                if code == 4100:
                                    code = 4260
                                    message = 'Invalid Area value on Index {0}.'
                                    message = message.format(i + 1)
                                else:
                                    message = message + ' Index ' + str(i + 1)
                                raise Exception
                        # Creating Polygon
                        sArea.append(sArea[0])

                        try:
                            yield self.serviceArea.insert(
                                    {
                                        'disabled': False,
                                        'name': tName,
                            ...
                            status = True
                            code = 2000
                            message = 'New Service Area has been created.'
                        except:
                            status = False
                            code = 4280
