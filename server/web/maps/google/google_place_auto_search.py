...
@xenSecureV1
class MmsWebMapsGooglePlaceAutoSearch(cyclone.web.RequestHandler, MongoMixin):
                ...
                if len(app):
                    ...
                    if self.apiId in [40216, 20216]: # TODO: till here
                        ...
                        API_URL = 'https://maps.googleapis.com/maps/api/place/autocomplete/json?' \
                                    + 'key={0}&input={1}&location={2},{3}&radius=10000' \
                        ...
                        if int(resp['status']) == 200:
                            ...
                            print rs
                            if len(rs['predictions']):
                                for idx, val in enumerate(rs['predictions']):
                                    v = {
                                            'id': val['place_id'],
                                            'name': val['structured_formatting']['main_text']
                                        }
                                    try:
                                        v['address'] = val['structured_formatting']['secondary_text']
                                    except:
                                        v['address'] = ''
