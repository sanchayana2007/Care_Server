...
ADMIN_RT_WS_CONNECTIONS = {
            'totalSocket': 0
        }

class MmsWebAdminRealtimeWebSocketHandler(cyclone.web.RequestHandler,
    ...
    def initialize(self):
        ...
        self.profileId = None
        self.time = timeNow()
        Log.i('[ ADMIN-SOC ]', 'Connection Request.')

    @defer.inlineCallbacks
    def _execute(self, transforms, *args, **kwargs):
        try:
            self.key = self.request.headers['Sec-Websocket-Key']
            Log.i('[ ADMIN-SOC ] Key', self.key)
            ...
            if not accountId:
                ...
                Log.i('[ ADMIN-SOC ] Authorization', self.accountId)
            ...
            if not entityId:
                ...
                Log.i('[ ADMIN-SOC ] x-Origin-Key', self.entityId)
            ...
            if not applicationId:
                ...
                Log.i('[ ADMIN-SOC ] x-Api-Key', self.applicationId)
            ...
            if len(profile):
                ...
                if len(app):
                    if app[0]['apiId'] == 20216: # TODO: till here
                        ...
                        Log.i('[ ADMIN-SOC ]', 'Initialized, Profile: ' + str(self.profileId))
                    else:
    ...
    def connectionMade(self, *args, **kwargs):
        try:
            ...
            Log.i('[ ADMIN-SOC ]', 'Connected.')
            '''
                Saving in a global Array
            '''
            '''
            oldConn = ADMIN_RT_WS_CONNECTIONS.get(self.profileId)
            if oldConn == None:
                oldConn =   {
                                'connection': [self]
                            }
            ...
            ADMIN_RT_WS_CONNECTIONS[self.profileId] = oldConn
            '''
            ADMIN_RT_WS_CONNECTIONS['totalSocket'] = ADMIN_RT_WS_CONNECTIONS['totalSocket'] + 1
            Log.i('[ ADMIN-SOC ]', 'Total Connection : ' + str(ADMIN_RT_WS_CONNECTIONS['totalSocket']))
        except Exception as e:
    ...
    def messageReceived(self, body=str):
        ...
        try:
            Log.d('[ ADMIN-SOC ] PID', self.profileId)
            try:
            ...
            if sCode == 2607191245:
                ...
                Log.i('[ ADMIN-SOC ] REDISH', 'SUBSCRIBE ChannelIDs: {0}'.format(
                        channelId
                    )
                )
                code = sCode
            ...
            # Unsubcribe all vehicles
                ...
                Log.i('[ ADMIN-SOC ] REDISH', 'UNSUBSCRIBE ChannelIDs: {0}'.format(
                        args['result']
                    )
                )
                code = sCode
        ...
        Log.d('[ ADMIN-SOC ] RESPONSE', response)
        try:
    ...
    def connectionLost(self, reason):
        ...
        '''
        '''
        oldConn = yield ADMIN_RT_WS_CONNECTIONS.get(self.profileId)
        if oldConn != None:
        ...
        '''
        if self.profileId:
            ADMIN_RT_WS_CONNECTIONS['totalSocket'] = \
                ADMIN_RT_WS_CONNECTIONS['totalSocket'] - 1
            self.redis.punsubscribe('*')
            self.redis.quit()

        Log.i('[ ADMIN-SOC ] LOST-Key', self.key)
        Log.i('[ ADMIN-SOC ] LOST-Authorization', self.accountId)
        Log.i('[ ADMIN-SOC ] LOST-x-Origin-Key', self.entityId)
        Log.i('[ ADMIN-SOC ] LOST-x-Api-Key', self.applicationId)
        Log.i('[ ADMIN-SOC ] LOST-Profile', self.profileId)
        Log.i('[ ADMIN-SOC ] LOST', 'Total Connection : ' + str(ADMIN_RT_WS_CONNECTIONS['totalSocket']))
        return
